...
function getSongsOverThreeMinutes(songs) {
  const songsOverThreeMins = songs.filter(song => (song.runtimeInSeconds > 180)) 
  return songsOverThreeMins;
}
...
function getTitleTracks(songs) {
  ...
  // })
  // return titleTracks;
  const titleTracks = songs.filter(song => (song.title === song.album)) 
  return titleTracks;
}
